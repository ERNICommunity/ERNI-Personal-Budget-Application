...
export class BudgetService {
  ...
  public getActiveUsersBudgets(year: number): Observable<Budget[]> {
    return this.http.get<Budget[]>(
      this.configService.apiUrlBase + this.url + 'users/active/year/' + year,
      this.serviceHelper.getHttpOptions(),
    );
  }

  public getCurrentUserBudgets(year: number): Observable<CurrentUsersBudget[]> {
  ...
  public transferBudget(budgetId: number, userId: number): Observable<unknown> {
    return this.http.put(
      this.configService.apiUrlBase + this.url + budgetId + '/transfer/' + userId,
      this.serviceHelper.getHttpOptions(),
    );
  }

  public createBudget(title: string, amount: number, userId: number, budgetType: number): Observable<unknown> {
  ...
  public createBudgetsForAllActiveUsers(title: string, amount: number, budgetType: number): Observable<unknown> {
    return this.http.post(
  ...
  public getBudgetsTypes(): Observable<BudgetType[]> {
    if (!this.#budgetTypes) {
      this.#budgetTypes = this.http
        .get<BudgetType[]>(this.configService.apiUrlBase + this.url + 'types', this.serviceHelper.getHttpOptions())
        .pipe(shareReplay(1));
    }
  ...
  public getUsersAvailableForBudgetType(budgetType: number): Observable<User[]> {
    return this.http.get<User[]>(
      this.configService.apiUrlBase + this.url + 'usersAvailableForBudgetType/' + budgetType,
      this.serviceHelper.getHttpOptions(),
    );
  }
}
