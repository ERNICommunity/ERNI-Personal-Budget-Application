...
export class OtherBudgetsComponent {
  ...
  years = computed(() => {
    ...
    return selectedBudgetType ? MenuHelper.getYearMenu((year) => ['/budgets/', year, selectedBudgetType]) : [];
  });

  budgetTypes$ = combineLatest([this.budgetService.getBudgetsTypes(), this.selectedYear$]).pipe(
    map(([types, year]) =>
      types.map((type) => ({
        label: type.name,
  ...
  #budgets = toSignal(
    combineLatest([this.selectedYear$, this.notificationService.notifications$]).pipe(
  ...
  budgets = computed(() => {
    ...
    return this.filterBudgets(this.#budgets(), this.searchTerm(), selectedBudgetType);
  });
